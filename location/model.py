...
def predict_rating(user_id, num_places=10):
    user_id = user_id
...
selected_columns = df_recom[['user_id','rating_review','place_id', 'name', 'address_city']]
...
dim_users = selected_columns['user_id'].max() + 1
...
history = model.fit(x=[train_data['user_id'],train_data['place_id']],
                    y=y_train,
                    batch_size=64,
                    epochs=10,
                    verbose=1,
                    validation_data=([valid_data['user_id'], valid_data['place_id']], y_val),
                    callbacks = [callback])


#Prediksi rating
result_df = predict_rating(1, 10)

print(result_df['Nama Tempat'])
